...
def health():
    ...
    try:
        ...
        checks["database"] = {"status": "degraded", "error": "Component is currently unavailable"}
    ...
    try:
        ...
        checks["model"] = {"status": "degraded", "error": "Component is currently unavailable"}
        status["status"] = "degraded"
    ...
    try:
        ...
        checks["cache"] = {"status": "degraded", "error": "Component is currently unavailable"}
        if status["status"] == "ok":
            status["status"] = "degraded"
...
def search_tools():
    ...
    try:
        ...
        return _error_response(
            "An internal error occurred while searching. Please try again later.",
            code="SEARCH_FAILED",
            retryable=True,
            status=500,
        )
