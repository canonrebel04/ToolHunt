...
def _load_tools():
    ...
    with _lock:
        ...
        for idx, desc in enumerate(descriptions):
            if desc not in descriptions_index:
                descriptions_index[desc] = idx
...
def find_indices(primary_list, query_list):
    ...
    """
    # ⚡ Bolt: Fast path using globally cached index map for the primary dataset to avoid O(N) dict building on every search
    if _descriptions is not None and primary_list is _descriptions:
        return [_descriptions_index[q] for q in query_list if q in _descriptions_index]

    # ⚡ Bolt: Fallback for arbitrary lists. Cache index lookup for O(1) retrieval.
    primary_dict = {}
