...
def _load_tools():
    ...
    with _lock:
        ...
        # Build index mapping for O(1) lookup during search
        # Converts O(N*M) list lookups to O(N+M), significantly reducing search latency for large DBs.
        mapping = {}
        for idx, val in enumerate(descriptions):
            if val not in mapping:
                mapping[val] = idx

        _description_to_index = mapping
...
def search_tool(query):
    ...
    # Collect the full tool data for each matching description using O(1) hash map lookup
    # Prevents O(N*M) performance penalty from nested list.index() calls, maintaining RRF order.
    matching_tools_data = []
