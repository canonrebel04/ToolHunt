...
def _load_tools():
    ...
    with _lock:
        ...
        # Build O(1) lookup map for fast hybrid search index resolution
        index_map = {}
        for idx, val in reversed(list(enumerate(descriptions))):
            index_map[val] = idx
        _descriptions_map = index_map

        # Assign _tools last in fast path variables
        _tools = tools
...
def find_indices(primary_list, query_list):
    ...
    # Fast path for module-level tool descriptions lookup
