...
def find_indices(primary_list, query_list):
    ...
    """
    # ⚡ Bolt Optimization: Build an O(1) lookup dictionary to avoid repeated O(N) primary_list.index() calls.
    # Improves time complexity from O(N*M) to O(N+M) for performance.
    lookup = {}
