...
def _load_tools():
    ...
    with _lock:
        ...
        # ⚡ Bolt: Precompute description-to-index mapping for O(1) lookup
        # Avoids rebuilding a 2,860-item dictionary on every search query
        descriptions_dict = {}
        for idx, item in enumerate(descriptions):
            if item not in descriptions_dict:
                descriptions_dict[item] = idx

        _descriptions_dict = descriptions_dict
        _descriptions = descriptions
        # Assign _tools last to prevent race condition where _tools is not None but _descriptions_dict is
        _tools = tools
...
def search_tool(query):
    ...
    # Collect the full tool data for each matching description using O(1) cached lookup
    matching_tools_data = []
    for desc in matching_descriptions:
        if desc in _descriptions_dict:
            index = _descriptions_dict[desc]
