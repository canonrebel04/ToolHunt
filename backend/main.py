...
def find_indices(primary_list, query_list):
    ...
    """
    # ⚡ Bolt Optimization: Replace O(N) list.index() lookup with O(1) dictionary lookup.
    # Impact: Reduces find_indices complexity from O(N*M) to O(N+M), dropping execution time significantly.
    index_map = {}
    ...
    for query_item in query_list:
        if query_item in index_map:
            indices.append(index_map[query_item])
    return indices
