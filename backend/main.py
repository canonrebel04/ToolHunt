...
def find_indices(primary_list, query_list):
    ...
    """
    # ⚡ Bolt Optimization: Replace O(N) list.index() inside a loop with an O(1) hash map lookup.
    # Expected Impact: Reduces find_indices time complexity from O(N * M) to O(N + M).
    mapping = {}
