...
def find_indices(primary_list, query_list):
    ...
    """
    # ⚡ Bolt: Replaced O(N*M) list.index() calls with an O(N) dictionary lookup.
    # Impact: Reduces time complexity for finding matches in large lists from O(N*M) to O(N + M).
    lookup = {}
