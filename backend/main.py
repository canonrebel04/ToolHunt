...
def find_indices(primary_list, query_list):
    ...
    # Replaced O(N^2) list.index() lookup with O(N) hash map lookup
    # Preserves first occurrence semantics of list.index()
    # Expected impact: >40% faster on large arrays (measurably drops lookups from O(N) to O(1))
    mapping = {}
