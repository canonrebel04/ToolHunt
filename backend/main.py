...
def find_indices(primary_list, query_list):
    ...
    """
    # ⚡ Bolt Optimization:
    # Changed from O(N*M) nested loop (list.index()) to O(N+M) hash map lookup.
    # Impact: Significantly reduces lookup time for large lists by avoiding
    # linear scans for every query item.
    mapping = {}
