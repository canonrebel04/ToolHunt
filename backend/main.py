...
def _load_tools():
    ...
    """
    global _tools, _descriptions, _descriptions_index_map
    ...
    with _lock:
        ...
        # Iterate in reverse to correctly mimic `.index()` returning the first occurrence
        index_map = {}
        for idx, val in reversed(list(enumerate(descriptions))):
            index_map[val] = idx

        _descriptions_index_map = index_map
...
def find_indices(primary_list, query_list):
    ...
    # Fast path for static module-level cache
    if primary_list is _descriptions and _descriptions_index_map is not None:
        for query_item in query_list:
            if query_item in _descriptions_index_map:
                indices.append(_descriptions_index_map[query_item])
        return indices
