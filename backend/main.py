...
def find_indices(primary_list, query_list):
    ...
    """
    # ⚡ Bolt Optimization: Replace O(M*N) nested loop with O(N + M) hash map lookup
    # using a dictionary. By preserving the first occurrence, we maintain correctness
    # while significantly reducing the time complexity for large lists.
    # Expected impact: Reduces find_indices time from O(N^2) to O(N).
    mapping = {}
