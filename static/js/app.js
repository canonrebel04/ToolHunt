...
function sanitizeUrl(url) {
    ...
    // SECURITY: Strip control characters and whitespace before testing protocol to prevent bypasses
    const testUrl = sanitized.replace(/[\x00-\x20]/g, '');
    if (/^(?:javascript|vbscript|data):/i.test(testUrl)) {
        return '#';
    }
    return sanitized;
}
