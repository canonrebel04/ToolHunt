    ...
    sortedTools.forEach((tool, index) => {
        ...
        toolCard.innerHTML = `
            ...
            <div class="card-footer">
                <a href="${tool.link || '#'}" target="_blank" class="tool-link" ${!tool.link ? 'style="opacity: 0.5; pointer-events: none;" aria-disabled="true" aria-label="No link available for ' + (tool.name || 'this tool') + '"' : 'aria-label="Access ' + (tool.name || 'this tool') + '"'}>
