    ...
    sortedTools.forEach((tool, index) => {
        ...
        toolCard.innerHTML = `
            ...
            <div class="card-footer">
                <a href="${tool.link || '#'}" target="_blank" class="tool-link" ${!tool.link ? 'style="opacity: 0.5; pointer-events: none;" aria-disabled="true"' : ''} aria-label="${tool.link ? `Access Tool: ${tool.name ? tool.name.replace(/"/g, '&quot;') : 'Unknown'}` : `No Link Available for ${tool.name ? tool.name.replace(/"/g, '&quot;') : 'Unknown'}`}">
