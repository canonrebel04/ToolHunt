    ...
    sortedTools.forEach((tool, index) => {
        ...
        toolCard.innerHTML = `
            ...
            <div class="card-footer">
                <a ${tool.link ? `href="${tool.link}"` : 'role="link" aria-disabled="true"'} target="_blank" class="tool-link" ${!tool.link ? 'style="opacity: 0.5; pointer-events: none;" tabindex="-1"' : ''} aria-label="${tool.link ? `Access Tool: ${tool.name}` : 'Link not available for ' + (tool.name || 'this tool')}">
