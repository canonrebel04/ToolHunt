    ...
    sortedTools.forEach((tool, index) => {
        ...
        toolCard.innerHTML = `
            ...
            <div class="card-footer">
                <a href="${tool.link || '#'}" target="_blank" class="tool-link" aria-label="${tool.link ? `Access ${tool.name || 'Unknown Tool'}` : 'No Link Available'}" ${!tool.link ? 'style="opacity: 0.5; pointer-events: none;" tabindex="-1" aria-disabled="true"' : ''}>
