    ...
    sortedTools.forEach((tool, index) => {
        ...
        toolCard.innerHTML = `
            ...
            <div class="card-footer">
                <a href="${tool.link || '#'}" target="_blank" class="tool-link" aria-label="${tool.link ? `Access ${tool.name || 'Tool'}` : `No link available for ${tool.name || 'Tool'}`}" ${!tool.link ? 'style="opacity: 0.5; pointer-events: none;" aria-disabled="true" tabindex="-1"' : ''}>
