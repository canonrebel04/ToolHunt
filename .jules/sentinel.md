...
**Vulnerability:** XSS bypass via control characters in URLs (e.g., `java	script:`).
**Learning:** Basic trimming is insufficient to prevent protocol-based XSS because browsers ignore control characters when parsing protocols.
**Prevention:** Strip all control characters (e.g., `[\x00-\x20]`) before validating and returning the URL.
